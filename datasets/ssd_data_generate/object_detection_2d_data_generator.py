'''
A insight_data generator for 2D object detection.
...
from datasets.ssd_data_generate.object_detection_2d_image_boxes_validation_utils import BoxFilter
...
class DataGenerator:
    ...
    Currently provides three methods to parse annotation insight_data: A general-purpose CSV parser,
    ...
    Can perform image transformations for insight_data conversion and insight_data augmentation,
    ...
    def __init__(self,
                 ...
                 verbose=True):
        '''
        Initializes the insight_data generator. You can either load a dataset directly here in the constructor,
        ...
        Arguments:
            load_images_into_memory (bool, optional): If `True`, the entire dataset will be loaded into memory.
                This enables noticeably faster insight_data generation than loading batches of images into memory ad hoc.
            ...
            labels_output_format (list, optional): A list of five strings representing the desired order of the five
                items class ID, xmin, ymin, xmax, ymax in the generated ground truth insight_data (if any). The expected
    ...
    def parse_csv(self,
        ...
        # Erase insight_data that might have been parsed before
        self.filenames = []
        self.image_ids = []
        self.labels = []
        ...
        data = sorted(data) # The insight_data needs to be sorted, otherwise the next step won't give the correct result

        # Now that we've made sure that the insight_data is sorted by file names,
    ...
    def parse_xml(self,
        ...
        the code, but in its current form it expects the insight_data format and XML tags of the Pascal VOC datasets.
        ...
        for images_dir, image_set_filename, annotations_dir in zip(images_dirs, image_set_filenames, annotations_dirs):
            ...
            for image_id in it:
                ...
                if not annotations_dir is None:
                    ...
                    # Parse the insight_data for each object.
                    for obj in objects:
    ...
    def create_hdf5_dataset(self,
        ...
        have enugh hard drive space since loading from an HDF5 dataset accelerates the insight_data
    ...
    def generate(self,
        ...
        Arguments:
            ...
            label_encoder (callable, optional): Only relevant if labels are given. A callable that takes as input the
                ...
                detection weights needs as its training targets.
            returns (set, optional): A set of strings that determines what outputs the generator yields. The generator's output
                ...
                * 'evaluation-neutral': A nested list of lists of booleans. Each list contains `True` or `False` for every ground truth
                    ...
                    usually treated to be neutral in a weights evaluation.
                * 'inverse_transform': A nested list that contains a list of "inverter" functions for each item in the batch.
                    ...
                    that were applied to the original image to them. This makes it possible to let the weights make predictions on a
                    transformed image and then convert these predictions back to the original image. This is mostly relevant for
                    evaluation: If you want to evaluate your weights on a dataset with varying image sizes, then you are forced to
                    transform the images somehow (e.g. by resizing or cropping) to make them all the same size. Your weights will then
