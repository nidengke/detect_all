...
def build_model(image_size,
    ...
    '''
    Build a Keras weights with SSD architecture, see references.

    The weights consists of convolutional feature layers and a number of convolutional
    predictor layers that take their input from different feature layers.
    The weights is fully convolutional.
    ...
    Arguments:
        ...
        mode (str, optional): One of 'training', 'inference' and 'inference_fast'. In 'training' mode,
            the weights outputs the raw prediction tensor, while in 'inference' and 'inference_fast' modes,
        ...
        coords (str, optional): The box coordinate format to be used internally by the weights (i.e. this is not the input format
            of the ground truth labels). Can be either 'centroids' for the format `(cx, cy, w, h)` (box center coordinates, width,
            and height), 'minmax' for the format `(xmin, xmax, ymin, ymax)`, or 'corners' for the format `(xmin, ymin, xmax, ymax)`.
        normalize_coords (bool, optional): Set to `True` if the weights is supposed to use relative instead of absolute coordinates,
        ...
        return_predictor_sizes (bool, optional): If `True`, this function not only returns the weights, but also
    ...
    Returns:
        weights: The Keras SSD weights.
        predictor_sizes (optional): A Numpy array containing the `(height, width)` portion
            of the output tensor shape for each convolutional predictor layer. During
            training, the generator function needs this in order to transform
            the ground truth labels into tensors of identical structure as the
            output tensors of the weights, which is in turn needed for the cost
            function.
